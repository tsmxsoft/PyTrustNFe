...
def _consultar(base_url,consulta,params = None, data = None):
    req = requests.get(base_url + "prefeitura/ws/nfse/consultar" + consulta, data=data, params=params, headers={'Content-Type': 'application/json'})
...
def _obj_send_parser(**kwargs):
    if not isinstance(kwargs.get('nfse'), dict):
        raise Exception("Objeto invalido")

    obj = kwargs.get('nfse')
    ret = []
    for rps in obj["lista_rps"]:
        rpsobj = {
            "cnpjPessoaPrestador": obj["cnpj_prestador"],
            ...
            "issRetidoPeloTomador": "SIM" if int(rps["servico"]["iss_retido"]) == 1 else "NÃO",
            ...
            "rpsDataEmissao": format_datetime_dmy(rps["data_emissao"]),
            "rpsSerie": rps["serie"],
            "rpsNumero": rps["numero"],
            "tokenRPS": obj["chave_digital"],
        }
        #CPF/CNPJ Tomador
