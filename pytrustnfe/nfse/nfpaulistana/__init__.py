...
def _render(certificado, method, **kwargs):
    ...
    if method in ["EnvioLoteRPS","EnvioLoteRpsAsync"]:
        ...
        kwargs["nfse"]["total_servicos"] = "{:.2f}".format(
            (sum(Decimal(rps["servico"]["valor_servico"]) for rps in kwargs["nfse"]["lista_rps"] if "valor_servico" in rps["servico"]) or Decimal("0.00"))
        )
