...
from pytrustnfe.nfse.prodata.assinatura import Assinatura
...
def _render(certificado, method, **kwargs):
    ...
    if method == "RecepcionarLoteRps" \
        ...
        for item in kwargs["nfse"]["lista_rps"]:
            ...
            xml_signed_send = signer.assina_xml(xml_send, reference, None)
        
        reference_lote = "lote:{0}".format(referencia)
        xml_signed_send = signer.assina_xml(xml_send, None, reference_lote)

        
    elif method == "CancelarNfse":
        print('cancelar')
        xml_signed_send = signer.assina_xml(xml_send,"rps:%s" %str(kwargs["nfse"]["rps"]["numero"]), None)
    else:
...
def _send(certificado, method, **kwargs):
    ...
    if ambiente == "producao" and not url:
        url = "https://morrinhos.prodataweb.inf.br/prodataws/services/NfseWSService?wsdl"
